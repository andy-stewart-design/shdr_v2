...
function explicitWrapperKind(init: AnyNode): DeclarationKind | null {
  ...
  if (callee.property === "let" || callee.property === "const")
...
function classifyName(
  name: string,
  boundaryKind: TransformBoundary["kind"],
...
export function collectExplicitNameEdits(
  ...
  for (const stmt of statements) {
    ...
    edits.push({
      name,
      insertPos: init.start + `${callee.object}.${callee.property}(`.length,
    });
  }
