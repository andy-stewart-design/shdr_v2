...
export const hash = fn("hash", [Vec2], Vec2, ([p], { vec2, dot, fract, sin }) => {
  const q = vec2(dot(p, vec2(2127.1, 81.17)), dot(p, vec2(1269.5, 283.37)));
  return fract(sin(q).mul(43758.5453));
});

export const noise = fn("noise", [Vec2], Float, ([p], { $, floor, fract, vec2, mix, dot }) => {
  const i = $.let("i", floor(p));
  const f = $.let("f", fract(p));
  const u = $.let("u", f.mul(f).mul(f.mul(2.0).sub(3.0).neg()));

  const g00 = hash(i.add(vec2(0.0, 0.0))).mul(2.0).sub(1.0);
  const g10 = hash(i.add(vec2(1.0, 0.0))).mul(2.0).sub(1.0);
  const g01 = hash(i.add(vec2(0.0, 1.0))).mul(2.0).sub(1.0);
  const g11 = hash(i.add(vec2(1.0, 1.0))).mul(2.0).sub(1.0);
